...
namespace _102200311_TranNhatMinh.BLL
{
    public class BLLQLSV
    {
        ...
        public List<SVHP_View> GetSVHP_ViewByidHP(string idHP)
        {
            List<SVHP_View> data = new List<SVHP_View>();
            var listSP_View = (idHP == "00000") ? from p in db.SinhVienHocPhans
                                                  select new { p._IDSV, p._IDHP, p._SinhVien._NameSV, p._SinhVien._LopSH, p._HocPhan._NameHP, p._DiemBT, p._DiemGK, p._DiemCK, p._NgayThi }
                :
                from p in db.SinhVienHocPhans
                where p._IDHP == idHP
                select new { p._IDSV, p._IDHP, p._SinhVien._NameSV, p._SinhVien._LopSH, p._HocPhan._NameHP, p._DiemBT, p._DiemGK, p._DiemCK, p._NgayThi };
            foreach (var v in listSP_View)
            {
                data.Add(new SVHP_View()
                {
                    _IDSV = v._IDSV,
                    _IDHP = v._IDHP,
                    _TenSV = v._NameSV,
        ...
        public SinhVienHocPhan GetSVHPByMSSV(string mssv)
        {
            SinhVienHocPhan sp = db.SinhVienHocPhans.Where(p => p._IDSV == mssv).FirstOrDefault();
            return sp;
        }
        public void Add_UpdateSVHP(SinhVienHocPhan svhp)
        {
            SinhVienHocPhan temp2 = db.SinhVienHocPhans.Find(svhp._IDSV, svhp._IDHP);
            if (temp2 == null)
                db.SinhVienHocPhans.Add(svhp);
            else
            {
                temp2._NgayThi = svhp._NgayThi;
                temp2._DiemBT = svhp._DiemBT;
                temp2._DiemGK = svhp._DiemGK;
                temp2._DiemCK = svhp._DiemCK;
            }
            db.SaveChanges();
        }
        ...
        public void DeleteSVHP(List<string> mssv, List<string> mshp)
        {
            for (int i = 0; i < mssv.Count; i++)
            {
                string MSSV = mssv[i];
                string MSHP = mshp[i];
                SinhVienHocPhan svhp = db.SinhVienHocPhans.Find(MSSV, MSHP);
                db.SinhVienHocPhans.Remove(svhp);
            }
            db.SaveChanges();
        }

        public List<SVHP_View> Search(string idhocphan, string text)
        {
            List<SVHP_View> data = new List<SVHP_View>();
            foreach (SVHP_View i in GetSVHP_ViewByidHP(idhocphan))
            {
        ...
        public List<SVHP_View> Sort(List<string> mssv, List<string> mshp, string typeSort)
        {
            List<SVHP_View> data = new List<SVHP_View>();
            for (int i = 0; i < mssv.Count; i++)
            {
                string MSSV = mssv[i];
                string MSHP = mshp[i];
                var listSPView = from p in db.SinhVienHocPhans
